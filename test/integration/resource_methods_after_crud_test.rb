...
class ResourceMethodsAfterCrudTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @match_new1 = { match: { date: "2021-04-10", score: 1, league_id: @league2.id, north: players(:player1).id, south: players(:player2).id, east: players(:player3).id, west: players(:player4).id } }
    @match_new2 = { match: { date: "2021-04-17", score: -2, league_id: @league2.id, north: players(:player2).id, south: players(:player3).id, east: players(:player4).id, west: players(:player5).id } }
